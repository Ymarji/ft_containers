...
namespace ft{
	...
	class	__tree_iter{
		...
		explicit __tree_iter(_nodePtr n):_node(n){};
		template <class Iter>
			__tree_iter (const __tree_iter<Iter, _nodePtr>& _it):_node(_it.base()){};
		_nodePtr base() const{
			return _node;
		};
	...
	class __tree{
		...
		public:
			node			*_end_node;
		public:
			__tree(const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()): _size(0), _CompObject(comp), _allocator(alloc) {
				_end_node = end_node();
			...
			void	insertHint(nodePtr Start, nodePtr newNode)
			{
				...
				if (Start != _tree_root && Start != _end_node)
				{
					nodePtr succ = ft::tree_next_iter(Start);
					nodePtr	pred = ft::tree_prev_iter(Start);
					if (_CompObject(succ->Value, newNode->Value))
						stat = false;
					if (pred != nullptr && _CompObject(newNode->Value, pred->Value))
						stat = false;
				}
				if (stat == true){
	...
	class map{
		...
		public:
			...
			explicit map (const key_compare& comp = key_compare(),
								const allocator_type& alloc = allocator_type()):_bst(tree(_CompObject, _allocator)), _CompObject(comp), _allocator(alloc) {};
			template <class InputIterator>
				map (InputIterator first, InputIterator last,
						const key_compare& comp = key_compare(),
						const allocator_type& alloc = allocator_type()):_bst(_CompObject), _CompObject(comp), _allocator(alloc){
							insert(first, last);
						};
			map (const map& x):_bst(_CompObject, _allocator), _CompObject(x._CompObject), _allocator(x._allocator){
				*this = x;
			};
